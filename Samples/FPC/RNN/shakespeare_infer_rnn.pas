...
const
  ...
  //SAVE_FILE='shakespeare.weights';
  SAVE_FILE='shakespeare.train.weights';
...
{$if defined(USE_OPENCL)}
  ...
  repeat
    for j:=0 to high(dev) do begin
      writeln(' ', j, ' [',TOpenCL.getDeviceTypeName(dev[j]),']: ', TOpenCL.getDeviceName(dev[j]));
      //writeln('          Supported extensions : ',TOpenCL.getDeviceExtensionStr(dev[j]).Replace(' ',#13#10'                                 '));
    end;
    try
      readln(j);
    except on e:Exception do
...
// my chaotic test arena
  ...
  //t4.reSize([B]);
  //
  //t1.UniformDistribution(-100, 100);
  //t2.UniformDistribution(-100, 100);
  //t1.copyTo(t3);
  //t3.add(t2);
  //
  //h1.reSize([B]);
  ...
  //h3.reSize([B]);
  //
  //writeln('half converting...');
  //SingleTpFP16(t1.Size(), t1.data, h1.Data);
  //SingleTpFP16(t2.Size(), t2.data, h2.Data);
  //h1.pushToDevice;
  //h2.pushToDevice;
  //
  //ocl.halfTest(h1.Size(), h1.devData, h2.devData, h3.devData);
  //h3.pullFromDevice;
  //FP16ToSingle(h3.Size(), h3.Data, t4.data);
  //t3.printStat;
  //t4.printstat;
  //
  //writeln('half converted');
  //
  //readln;
